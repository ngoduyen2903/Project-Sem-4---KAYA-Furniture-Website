...
public class ChartJsView implements Serializable {
    ...
    private BarChartModel barModel_1;

    private BarChartModel revenueProduct;
    private BarChartModel revenueCustomer;
    ...
    public String loadChart() {
        ...
        try {
            ...
            List<Orders> list = ordersFacade.findAllByDate(startConvert, endConvert);
            int quantityProductSold = getProductSold();
            int getCustomerPurchased = getCustomerPurchased();
            System.out.println("===1>" + list.size());
            for (Orders orders : list) {
                totalPrice += orders.getTotalPrice().doubleValue();
            }
            System.out.println("-1----" + totalPrice);
            createBarModel(quantityProductSold, getCustomerPurchased, totalPrice);
    ...
    private int getCustomerPurchased() {
        int quantitySoldCustomerPurchased = 0;
    ...
    public void revenuByMonth() {
        List<RevenueByMonth> list = getData();

        revenueByMonth = new BarChartModel();
        ChartData data = new ChartData();
        ...
        List<Number> values = new ArrayList<>();
        for (RevenueByMonth revenueByMonth : list) {
            double value = 0;
            for (Orders orders : ordersFacade.findAll()) {
                if (orders.getOrderDate().toLocalDate().getMonthValue() == revenueByMonth.getMonth()) {
                    value += orders.getTotalPrice().doubleValue();
                }
            }
            values.add(value);
        }
        ...
        List<String> labels = new ArrayList<>();
        for (RevenueByMonth revenueByMonth : list) {
            labels.add(revenueByMonth.getMonth() + "");
        }

        data.setLabels(labels);
        revenueByMonth.setData(data);
        ...
        revenueByMonth.setOptions(options);
    }
    ...
    public void revenuProductChart() {
        ...
        for (Products products : listProduct) {
            if (!products.getOrderDetailsCollection().isEmpty()) {
                values.add(products.getOrderDetailsCollection().size());
            } else {
                values.add(0);
            }
        }
    ...
    public void revenuCustomer() {
        List<Customers> list = customersFacade.findAll();
        revenueCustomer = new BarChartModel();
        ChartData data = new ChartData();

        BarChartDataSet barDataSet = new BarChartDataSet();
        barDataSet.setLabel("Quantity");
        List<Number> values = new ArrayList<>();
        for (Customers customers : list) {
            if (!customers.getOrdersCollection().isEmpty()) {
                values.add(customers.getOrdersCollection().size());
            } else {
                values.add(0);
            }
        }
        ...
        List<String> labels = new ArrayList<>();
        for (Customers customers : list) {
            labels.add(customers.getUsername());
        }

        data.setLabels(labels);
        revenueCustomer.setData(data);
        ...
        revenueCustomer.setOptions(options);
    }
    ...
    public BarChartModel getBarModel_1() {
        return barModel_1;
    }

    public void setBarModel_1(BarChartModel barModel_1) {
        this.barModel_1 = barModel_1;
    }
    ...
    public BarChartModel getRevenueCustomer() {
        return revenueCustomer;
    }

    public void setRevenueCustomer(BarChartModel revenueCustomer) {
        this.revenueCustomer = revenueCustomer;
    }
